...
int main()
{
	...
	// observer.generateParticles<ObserverParticleGenerator>(observation_location);
	//----------------------------------------------------------------------
	...
	// ContactRelation observer_contact(observer, {&water_block});
	//----------------------------------------------------------------------
	...
	SimpleDynamics<DropletInitialSpeed> initial_condition(water_block);
	...
	// Dynamics1Level<fluid_dynamics::MultiPhaseIntegration1stHalfRiemann>
	// 	water_pressure_relaxation(water_air_complex);
	// Dynamics1Level<fluid_dynamics::MultiPhaseIntegration2ndHalfRiemann>
	// 	water_density_relaxation(water_air_complex);
	...
	// InteractionDynamics<fluid_dynamics::MultiPhaseColorFunctionGradient> color_gradient(water_air_complex.getContactRelation());
	...
	// InteractionWithUpdate<fluid_dynamics::FreeSurfaceIndicationComplex> surface_detection(water_air_complex);
	// InteractionDynamics<fluid_dynamics::ColorFunctionGradientComplex> color_gradient(water_air_complex);
	// InteractionDynamics<fluid_dynamics::ColorFunctionGradientInterpolationInner> color_gradient_interpolation(water_air_complex.getInnerRelation());
	// InteractionDynamics<fluid_dynamics::SurfaceTensionAccelerationInner> surface_tension_acceleration(water_air_complex.getInnerRelation(), tension_force);

	// RegressionTestDynamicTimeWarping<ObservedQuantityRecording<Vecd>>
		// write_centre_of_mass_displacement("Position", io_environment, observer_contact);
	CustomReduceDynamics<fluid_dynamics::CenterOfMass> write_centre_of_mass_pos(water_block);
	...
	/** Create a dat file for storing COM of the upper left quarter of water block at every output interval */
	PltEngine plt_engine_;
	fluid_dynamics::CenterOfMass com(water_block);
	/** output for .dat file. */
	std::string filefullpath_output_ = io_environment.output_folder_ + "/" +  "COM_position" + ".dat";
	std::ofstream out_file(filefullpath_output_.c_str(), std::ios::app);
	out_file << "\"run_time\""<< "   ";
	plt_engine_.writeAQuantityHeader(out_file, com.Reference(), "COM_pos");
	out_file << "\n";
	// out_file.close();
	//----------------------------------------------------------------------
	...
	// Real output_interval_2 = end_time_1/100;
	...
	while (GlobalStaticVariables::physical_time_ < end_time)
	{
		...
		while (integration_time < output_interval)
		{
			...
			delta_t1 = 0.25*pow(0.1*rho0_f*pow(1.3*particle_spacing_ref,3)/(2*M_PI*tension_force),0.5);
			...
			delta_t3 = SMIN(Dt_acc_f, Dt_acc_a);
			// delta_t3 = 0.25*pow(1.3*particle_spacing_ref/gravity_g, 0.5);
			delta_t4 = 0.125*pow(1.3*particle_spacing_ref,2)/mu_f;
	...
	// Extra loop for simulation beyond 0.3 s
	// while (GlobalStaticVariables::physical_time_ < end_time_2)
	// {
	// 	Real integration_time = 0.0;
	// 	/** Integrate time (loop) until the next output time. */
	// 	while (integration_time < output_interval_2)
	// 	{
	// 		/** Acceleration due to viscous force and gravity. */
	// 		time_instance = TickCount::now();
	...
	// 		// Real Dt = SMIN(Dt_f, Dt_a);
	// 		delta_t1 = 0.25*pow(0.1*rho0_f*pow(1.3*particle_spacing_ref,3)/(2*M_PI*tension_force),0.5);
	// 		Real Dt_sound_f = get_water_sound_time_step_size.parallel_exec();
	// 		Real Dt_sound_a = get_air_sound_time_step_size.parallel_exec();
	// 		delta_t2 = SMIN(Dt_sound_f, Dt_sound_a);
	// 		Real Dt_acc_f = get_water_acc_time_step_size.parallel_exec();
	// 		Real Dt_acc_a = get_air_acc_time_step_size.parallel_exec();
	// 		delta_t3 = SMIN(Dt_acc_f, Dt_acc_a);
	// 		// delta_t3 = 0.25*pow(1.3*particle_spacing_ref/gravity_g, 0.5);
	// 		delta_t4 = 0.125*pow(1.3*particle_spacing_ref,2)/mu_f;
	// 		Real Dt = 0.01 * SMIN(Dt_f,Dt_a,delta_t1,delta_t2,delta_t3,delta_t4);
	...
	// 		// std::ofstream out_file(filefullpath_output_.c_str(), std::ios::app);
	// 		out_file << GlobalStaticVariables::physical_time_ << "   ";
	// 		plt_engine_.writeAQuantity(out_file, write_centre_of_mass_pos.parallel_exec());
	// 		out_file << "\n";
	// 		// out_file.close();
	...
	// {
	// 	write_centre_of_mass_pos.generateDataBase(1.0e-2);
	// }
	// else
	// {
	//  	write_centre_of_mass_pos.newResultTest();
	// }
	return 0;
}

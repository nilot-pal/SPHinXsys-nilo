...
/**
* @class CustomReduceDynamics
* @brief Template class for particle-wise reduce operation, summation, max or min.
*/
template <class LocalDynamicsType, class ExecutionPolicy = ParallelPolicy>
class CustomReduceDynamics : public LocalDynamicsType,
public BaseDynamics<typename LocalDynamicsType::ReduceReturnType>
{
	using ReturnType = typename LocalDynamicsType::ReduceReturnType;
	using PartialReturnType = typename LocalDynamicsType::PartialReduceReturnType;

	public:
		template <class DynamicsIdentifier, typename... Args>
		CustomReduceDynamics(DynamicsIdentifier &identifier, Args &&...args)
			: LocalDynamicsType(identifier, std::forward<Args>(args)...),
			  BaseDynamics<ReturnType>(identifier.getSPHBody()){};
		virtual ~CustomReduceDynamics(){};

		using ReduceReturnType = ReturnType;
		std::string QuantityName() { return this->quantity_name_; };
		std::string DynamicsIdentifierName() { return this->identifier_.getName(); };
		/** The custom sequential function for executing the reduce operations on particles. */
		virtual ReturnType exec(Real dt = 0.0) override
		{
			this->setupDynamics(dt);
			ReturnType temp = particle_reduce(ExecutionPolicy(),
				this->identifier_.LoopRange(), this->Reference(), this->getOperation2(),
				[&](size_t i) -> ReturnType
				{ return this->reduce(i, dt); });
			PartialReturnType partialTemp = particle_reduce(ExecutionPolicy(),
				this->identifier_.LoopRange(), 0.0, this->getOperation1(),
				[&](size_t i) -> PartialReturnType
				{ return this->scalarReduce(i, dt); });
			return this->outputResult(temp, partialTemp);
		};
	};

...
/**
	 * @class MyBaseLocalDynamicsReduce
	 * @brief Customized version of base class for all local particle dynamics. This
	 * can account for different types in the return type and the argument type in 
	 * method outputResult
	 */
	template <typename ReturnType1, typename ReturnType2, typename Operation1, typename Operation2, class DynamicsIdentifier>
	class MyBaseLocalDynamicsReduce : public BaseLocalDynamics<DynamicsIdentifier>
	{
	public:
		MyBaseLocalDynamicsReduce(DynamicsIdentifier &identifier, ReturnType2 reference)
			: BaseLocalDynamics<DynamicsIdentifier>(identifier), reference_(reference),
			  quantity_name_("ReducedQuantity"){};
		virtual ~MyBaseLocalDynamicsReduce(){};

		using ReduceReturnType = ReturnType2;
		using PartialReduceReturnType = ReturnType1;
		ReturnType2 Reference() { return reference_; };
		std::string QuantityName() { return quantity_name_; };
		Operation1 &getOperation1() { return operation1_; };
		Operation2 &getOperation2() { return operation2_; };
		virtual ReturnType2 outputResult(ReturnType2 reduced_value, ReturnType1 scalar_reduced_value) 
		{
			ReturnType2 result;
			return result;
		}
	...
	protected:
		ReturnType2 reference_;
		Operation1 operation1_;
		Operation2 operation2_;
		std::string quantity_name_;
	};
	
template <typename ReturnType1, typename ReturnType2, typename Operation1, typename Operation2>
using MyLocalDynamicsReduce = MyBaseLocalDynamicsReduce<ReturnType1, ReturnType2, Operation1, Operation2, SPHBody>;

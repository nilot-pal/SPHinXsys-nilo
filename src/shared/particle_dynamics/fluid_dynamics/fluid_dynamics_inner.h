...
/**
* @class SoundSpeedTimeStepSize
* @brief Computing the time step size according to sound speed criterion (required for oscillating droplet)
*/
class SoundSpeedTimeStepSize : public LocalDynamicsReduce<Real, ReduceMax>, public FluidDataSimple
{
	public:
		explicit SoundSpeedTimeStepSize(SPHBody &sph_body, Real soundCFL = 0.25);
		virtual ~SoundSpeedTimeStepSize(){};
		Real reduce(size_t index_i, Real dt = 0.0);
		virtual Real outputResult(Real reduced_value) override;
...
/**
* @class ParticleAccelerationTimeStepSize
* @brief Computing the time step size based on individual particle acceleration criterion (required for oscillating droplet)
*/
class ParticleAccelerationTimeStepSize : public LocalDynamicsReduce<Real, ReduceMax>, public FluidDataSimple
{
	public:
		explicit ParticleAccelerationTimeStepSize(SPHBody &sph_body, Real accelerationCFL = 0.25);
		virtual ~ParticleAccelerationTimeStepSize(){};
		Real reduce(size_t index_i, Real dt = 0.0);
		virtual Real outputResult(Real reduced_value) override;
	...
	protected:
		...
		Real smoothing_length_min_;
		Real accelerationCFL_;
};
		
/**
* @class center of mass
* @brief Computing the center of mass
*/
class CenterOfMass : public MyLocalDynamicsReduce<Real, Vecd, ReduceSum<Real>, ReduceSum<Vecd>>, public FluidDataSimple
{
	public:
		explicit CenterOfMass(SPHBody &sph_body);
		virtual ~CenterOfMass(){};
		Vecd reduce(size_t index_i, Real dt = 0.0);
		Real scalarReduce(size_t index_i, Real dt = 0.0);
		Vecd outputResult(Vecd reduced_value, Real scalar_reduced_value);

...
//=================================================================================================//
SoundSpeedTimeStepSize::SoundSpeedTimeStepSize(SPHBody &sph_body, Real soundCFL)
	: LocalDynamicsReduce<Real, ReduceMax>(sph_body, Real(0)),
	...
	soundCFL_(soundCFL) {}
	//=================================================================================================//
Real SoundSpeedTimeStepSize::reduce(size_t index_i, Real dt)
{
	return fluid_.getSoundSpeed(p_[index_i], rho_[index_i]);
}
//=================================================================================================//
Real SoundSpeedTimeStepSize::outputResult(Real reduced_value)
{
	return soundCFL_ * smoothing_length_min_ / (reduced_value + TinyReal);
}
//=================================================================================================
ParticleAccelerationTimeStepSize::ParticleAccelerationTimeStepSize(SPHBody &sph_body, Real accelerationCFL)
	: LocalDynamicsReduce<Real, ReduceMax>(sph_body, Real(0)),
	FluidDataSimple(sph_body), fluid_(DynamicCast<Fluid>(this, particles_->getBaseMaterial())), rho_(particles_->rho_),
	p_(*particles_->getVariableByName<Real>("Pressure")), vel_(particles_->vel_), acc_(particles_->acc_),
	smoothing_length_min_(sph_body.sph_adaptation_->MinimumSmoothingLength()),
	accelerationCFL_(accelerationCFL) {}
		//=================================================================================================//
Real ParticleAccelerationTimeStepSize::reduce(size_t index_i, Real dt)
{
	return acc_[index_i].squaredNorm();
}
		//=================================================================================================//
Real ParticleAccelerationTimeStepSize::outputResult(Real reduced_value)
{
	Real acc_max = sqrt(reduced_value);
	return accelerationCFL_* sqrt(smoothing_length_min_/ (acc_max + TinyReal));
}
		
CenterOfMass::CenterOfMass(SPHBody &sph_body)
	: MyLocalDynamicsReduce<Real, Vecd, ReduceSum<Real>, ReduceSum<Vecd>>(sph_body, Vecd::Zero()),
FluidDataSimple(sph_body), fluid_(DynamicCast<Fluid>(this, particles_->getBaseMaterial())), pos_(particles_->pos_),
rho_(particles_->rho_), Vol_(particles_->Vol_){}
		//=================================================================================================//
Vecd CenterOfMass::reduce(size_t index_i, Real dt)
{
	Vecd value = Vecd::Zero();
	if(pos_[index_i][0]<0 && pos_[index_i][1]>0) {
		value = pos_[index_i]*rho_[index_i]*Vol_[index_i];
	}
	else 
	{
		value = Vecd::Zero();	
...
Real CenterOfMass::scalarReduce(size_t index_i, Real dt)
{
	Real value = 0.0;
	if(pos_[index_i][0]<0 && pos_[index_i][1]>0) {
		value = rho_[index_i]*Vol_[index_i];
	}
	else 
	{
		value = 0.0;	
	...
	return value;
		...
		//=================================================================================================//
Vecd CenterOfMass::outputResult(Vecd reduced_value, Real scalar_reduced_value)
{
	Vecd numerator = reduced_value;
	numerator[0] = numerator[0]/scalar_reduced_value;
	numerator[1] = numerator[1]/scalar_reduced_value;
	return numerator;
}
